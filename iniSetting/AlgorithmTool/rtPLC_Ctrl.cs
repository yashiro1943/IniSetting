...
namespace AlgorithmTool
{
    ...
    public struct rtMotor_Cfg
    {
        ...
        public void LoadDefault()
        {
            ...
            Ini PLC_Ini = new Ini();
            PLC_Ini.Read_ini_Cfg1();
            tPID_PowerCoe.eKp = PLC_Ini.PID_POWER_COE_KP;
            tPID_ThetaOffsetCoe.eKp = PLC_Ini.PID_THETA_OFFSET_COE_KP;
            tPID_MotorAngleCoe.eKp = PLC_Ini.PID_ANGLE_CAR_MOTOR_COE_KP;
            lRotationDistance = PLC_Ini.RADIUS_SMOOTH;
        }
    }
    ...
    public class rtMotorCtrl
    {
        ...
        {   // 直走狀態
            ...
            Ini Radius_Smooth = new Ini();
            Radius_Smooth.Read_ini_Cfg1();
            ...
            {   // 尚未超過終點
                ...
                switch (a_atPathInfo[tMotorData.lPathNodeIndex].ucTurnType)
                {
                    ...
                    case (byte)rtPath_Info.rtTurnType.SMOOTH:
                        if (eDistanceTurn < Radius_Smooth.RADIUS_SMOOTH)
                        {
                            // 算出旋轉半徑 & 中心座標
