...
namespace AlgorithmTool
{
    ...
    public struct rtCarCFG
    {
        ...
        {
            Ini AGV_Ini = new Ini();
            AGV_Ini.Read_ini_Cfg1();
            lLength = AGV_Ini.CAR_LENGTH;
            lWidth = AGV_Ini.CAR_WIDTH_WHEEL;
        }
    }

    /// <summary> Struct of Intelligent-Storage-System </summary>
    /// <remarks> 
    ...
    public struct rtAGV_CFG
    {
        ...
        /** \brief Map 自走車設定   */
        public rtVehicleCfg tVehicleCfg;
        ...
        {
            tMapCfg.Init();
            tVehicleCfg.LoadDefault();
        }
    }

    /// <summary> the configure struct of Vehicle </summary> 
    public struct rtVehicleCfg
    {
        /// <summary> Vehicle ID </summary>
        public int lID;

        /// <summary> Spec of Vehicle 車身規格設定 </summary>
    ...
    public class rtAGV_Control
    {
        ...
        public int rtAGV_Navigation(RegionOfMap a_tRegionData, int a_lWarehousIndex, ROI[] a_atObstacle, bool a_bPark)
        {   // 沒路徑 離終點太近無法校正等都會回傳-1 表示錯誤
            int lCheckResult = 1;

            rtWarehouseInfo tWarehouse = a_tRegionData.atWarehouseCfg[a_lWarehousIndex];
            ...
            if (tAGV_Data.atPathInfo.Length <= 0 || a_atObstacle.Length != 0)
            {
                ...
                rtPathPlanning.rtAGV_PathPlanning(
                    a_tRegionData, a_lWarehousIndex,
                    ref tAGV_Data.atPathInfo, ref tAGV_Data.tCarInfo, a_atObstacle);
                ...
                {   // 如果沒路徑就不需修正
                    ...
                    PathModifyForStorage(ref tAGV_Data.atPathInfo, tAGV_Data.tCarInfo, tWarehouse.eDirection, a_bPark);
                }

                // 判斷是否已到達終點
                lCheckResult = ArriveCheck(tAGV_Data.atPathInfo, tWarehouse);
            }
                ...
                switch (tAGV_Data.ucAGV_Status)
                {
                    ...
                    case (byte)rtAGVStatus.ALIMENT:
                        bDone = tAGV_Data.CMotor.CarAngleAlignment(
                            tIS_Cfg.tMapCfg.atRegionCfg[tWarehousPos.lRegion].atWarehouseCfg[tWarehousPos.lIndex].eDirection,
                            tAGV_Data.tCarInfo);

                        if (bDone)
                        {   // 達到要的角度 >>  進入 PARKING 動作
                            rtVector tDirectionLocal = new rtVector();
                            tDirectionLocal = rtVectorOP_2D.Angle2Vector(tIS_Cfg.tMapCfg.atRegionCfg[tWarehousPos.lRegion].atWarehouseCfg[tWarehousPos.lIndex].eDirection);
                            ...
                            tAGV_Data.atPathInfoForkForth[0].tDest.eX = tIS_Cfg.tMapCfg.atRegionCfg[tWarehousPos.lRegion].atWarehouseCfg[tWarehousPos.lIndex].tCoordinate.eX;
                            tAGV_Data.atPathInfoForkForth[0].tDest.eY = tIS_Cfg.tMapCfg.atRegionCfg[tWarehousPos.lRegion].atWarehouseCfg[tWarehousPos.lIndex].tCoordinate.eY;
        ...
        /// <param name="a_tWarehouseData">[IN] destination Information</param>
        /// <returns> 0: close enough 1: nothing wrong -1: error </returns>
        private int ArriveCheck(rtPath_Info[] a_atPathInfo, rtWarehouseInfo a_tWarehouseData)
        {
            double eDistance = 0;
        ...
        public void AutoNavigate(WarehousPos a_tWarehousPos, bool a_bPark)
        {
            ...
            int lCheckResult = 0;
            RegionOfMap tRegionData;

            // 從cfg中找出 目前區域設定
            tRegionData = tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion];
            ...
            {
                // 檢測或算出路徑
                lCheckResult = rtAGV_Navigation(tRegionData, a_tWarehousPos.lIndex, atObstacle, a_bPark);
            ...
            while (tAGV_Data.CFork.tForkData.ucStatus != (byte)rtForkCtrl.ForkStatus.FINISH && tAGV_Data.ucAGV_Status != (byte)rtAGVStatus.EMERGENCY_STOP)
            {
                switch (tAGV_Data.CFork.tForkData.ucStatus)
                {
                    ...
                    case (byte)rtForkCtrl.ForkStatus.ALIMENT:
                        bDone = false;
                        bDone = tAGV_Data.CMotor.CarAngleAlignment(
                            tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].eDirection,
                            tAGV_Data.tCarInfo);
                        ...
                        {   // UNLOAD要高一點
                            tAGV_Data.CFork.tForkData.height = (int)tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].eHeight + rtForkCtrl.FORK_PICKUP_HEIGHT;
                        }
                        else if(tAGV_Data.ucAGV_Status == (byte)rtAGVStatus.LOAD)
                        {
                            tAGV_Data.CFork.tForkData.height = (int)tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].eHeight;
                        }
                        else
                        {   // error
                        ...
                        {   // 達到要的深度跟高度 >> 進入FORTH動作
                            rtVector tDirectionLocal = new rtVector();
                            tDirectionLocal = rtVectorOP_2D.Angle2Vector(tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].eDirection);
                            tDirectionLocal = rtVectorOP_2D.VectorMultiple(tDirectionLocal, -1);
                            ...
                            tAGV_Data.atPathInfoForkForth[0].tDest.eX = tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].tCoordinate.eX;
                            tAGV_Data.atPathInfoForkForth[0].tDest.eY = tIS_Cfg.tMapCfg.atRegionCfg[a_tWarehousPos.lRegion].atWarehouseCfg[a_tWarehousPos.lIndex].tCoordinate.eY;
